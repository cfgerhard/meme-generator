...
class Ingestor(IngestorInterface):
    ...
    def can_ingest(cls, path: str) -> bool:
        """Check Ingestion capability.

        Return a bool if the parent class has support for
        file extension in a given path.
        """
    ...
    def parse(cls, path: str) -> List[QuoteModel]:
        ...
        if not cls.can_ingest(path):
            raise ValueError(f'Unsupported file extension '
                             f'"{extension}" from {path}. '
                             f'Supported types are ->'
                             f' {base.extension_support()}')

        parser = next((c for c in base.__subclasses__()
                       if extension in c.extensions), None)
